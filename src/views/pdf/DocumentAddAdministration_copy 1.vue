    ...
    <form @submit.prevent="submitForm">
      ...
      <div class="ocmopt-col cols1 mb-15">
              ...
              <div class="ocmopt-meta">
                         ...
                         <span>{{ option.people.lastname + " " + option.people.firstname }}</span>
											</div>
										</template>
...
export default {
  setup(){
    ...
    function readChildsOrganization(organizationParentId=2,organizaitons=ref([])){
      store.dispatch('organization/listByParent',{
        search: '' ,
        perPage: 1000 ,
        page: 1 ,
        id: organizationParentId // Parent Organization ID
      }).then( res => {
        organizaitons.value = res.data.records
      }).catch( error => {
    ...
    onMounted( () => {
      readChildsOrganization(1,ministries)
      readChildsOrganization(2,departments)
      getLeaders()
    })
